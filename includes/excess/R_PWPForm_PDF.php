...
$_SESSION["ObjCode"]=$ObjCode;
$_SESSION["key"]=$key;
//$comment=$_POST["comment"];
/* $Requestor='KIEFER RAVENA';
$RequestorID=""; */

$Requestor=mysqli_real_escape_string($db, $_POST["Requestor"]);
$RequestorID=mysqli_real_escape_string($db, $_POST["RequestorID"]);


if($RequestorID==="" || $RequestorID===null)
{
	$getrequestorID="SELECT * FROM `OUSR` where CONCAT (`FirstName`,' ',`LastName`)='$Requestor'";
	$rrequestorID=mysqli_query($db, $getrequestorID);
	if(mysqli_num_rows($rrequestorID)>0)
	{
		while($rRID = mysqli_fetch_array($rrequestorID))
		{
			//$_SESSION["checker1"]=$rAID["ASM"];
			$_SESSION["RequestorID"]=$rRID["ID"];
			$RequestorID=$rRID["ID"];
		}
...
/*  $Requestor="KIEFER RAVENA";
$RequestorID="0000-002";  */

$_SESSION["Requestor"]=$Requestor;


$getPWPDetails="SELECT * 
FROM `OPWP`
left join `PWP1` on `PWP1`.`DocId`= `OPWP`.`DocId` and `PWP1`.`Document`= `OPWP`.`Document`
where `OPWP`.`DocId` ='$key' and `OPWP`.`Document`='$ObjCode'";
$rgetPWPDetails=mysqli_query($db, $getPWPDetails);
if(mysqli_num_rows($rgetPWPDetails)>0)
{
	while($rPWP = mysqli_fetch_array($rgetPWPDetails))
	{
		
		$_SESSION["PWPDate"]=$rPWP["PWPDate"];
		$_SESSION["AccountName"]=$rPWP["AccountName"];
		$_SESSION["ChannelClass"]=$rPWP["ChannelClass"];
		$_SESSION["Territory"]=$rPWP["Territory"];
		$_SESSION["ActivityType"]=$rPWP["ActivityType"];
		$_SESSION["2021Target"]=$rPWP["2021Target"];
		$_SESSION["TradeDisc"]=$rPWP["TradeDisc"];
		$_SESSION["DA"]=$rPWP["DA"];
		$_SESSION["PWPSeries"]=$rPWP["PWPSeries"];
		$_SESSION["CostCenter"]=$rPWP["CostCenter"];
		$_SESSION["PromoFrom"]=$rPWP["PromoFrom"];
		$_SESSION["PromoTo"]=$rPWP["PromoTo"];
		$_SESSION["PostPromoEval"]=$rPWP["PostPromoEval"];
		$_SESSION["YTD2021"]=$rPWP["YTD2021"];
		$_SESSION["YTD2022"]=$rPWP["YTD2022"];
		$_SESSION["Variance"]=$rPWP["Variance"];
		$_SESSION["Prcntge"]=$rPWP["Prcntge"];
		$_SESSION["Background"]=$rPWP["Background"];
		$_SESSION["Promotitle"]=$rPWP["Promotitle"];
		$_SESSION["Concession"]=$rPWP["Concession"];
		$_SESSION["PVATotal"]=$rPWP["PVATotal"];
		$_SESSION["PCTotal"]=$rPWP["PCTotal"];
		$_SESSION["PCETotal"]=$rPWP["PCETotal"];
		$_SESSION["TotProjCost"]=$rPWP["TotProjCost"];
		$_SESSION["ProjectedSales"]=$rPWP["ProjectedSales"];
		$_SESSION["ProjectedCost"]=$rPWP["ProjectedCost"];
		$_SESSION["Ratio"]=$rPWP["Ratio"];
...
//echo $ipwpdate;
/* 
$trAMSInCase=$rPWP["PWPDate"];
$trAMSPeso=$rPWP["PWPDate"];
$trTargetInCase=$rPWP["PWPDate"];
$trTargetPeso=$rPWP["PWPDate"];
$trProjectedInCase=$rPWP["PWPDate"];
$trPC_QTY=$rPWP["PWPDate"];
$trPC_UnitCost=$rPWP["PWPDate"];
$trPC_EXPQTY=$rPWP["PWPDate"];
$trPC_EXPUnitCost=$rPWP["PWPDate"]; 
$txtMechanics=$_POST["txtMechanics"];
$txtObjectives=$_POST["txtObjectives"];
$txtSKUS=$_POST["txtSKUS"];
$txtAMSInCase=$_POST["txtAMSInCase"];
$txtAMSPeso=$_POST["txtAMSPeso"];
$txtTargetInCase=$_POST["txtTargetInCase"];
$txtTargetPeso=$_POST["txtTargetPeso"];
$txtProjectedInCase=$_POST["txtProjectedInCase"];
$txtProjectedPeso=$_POST["txtProjectedPeso"];
$txtPC_Desc=$_POST["txtPC_Desc"];
$txtPC_Qty=$_POST["txtPC_Qty"];
$txtPC_UnitCost=$_POST["txtPC_UnitCost"];
$txtPC_TotalCost=$_POST["txtPC_TotalCost"];
$txtPC_ExpsDesc=$_POST["txtPC_ExpsDesc"];
$txtPC_ExpQTY=$_POST["txtPC_ExpQTY"];
$txtPC_ExpUnitCost=$_POST["txtPC_ExpUnitCost"];
$txtPC_ExpTotalCost=$_POST["txtPC_ExpTotalCost"];
 */

$getASMID="SELECT * FROM `OUSR` where `ID`='$RequestorID'";
$rASMID=mysqli_query($db, $getASMID);
if(mysqli_num_rows($rASMID)>0)
{
	while($rAID = mysqli_fetch_array($rASMID))
	{
		//$_SESSION["checker1"]=$rAID["ASM"];
		
		$getASMName="SELECT distinct CONCAT (`FirstName`,' ',`LastName`) as'Name',A.`ID`,C.* FROM `OUSR` A
								left join OPWP C on  C.`CheckedBy`=A.`ID`
								where A.`ID`='".$rAID["ASM"]."' and C.DocId='$key' ";
		$rASMName=mysqli_query($db, $getASMName);
		if(mysqli_num_rows($rASMName)>0)
		{
			while($rAName= mysqli_fetch_array($rASMName))
			{
				$_SESSION["checker1"]=$rAName["Name"];
				
				if($rAID["CheckedBy_status"]=="0" || $rAName["CheckedBy_status"]==0)
				{
					$_SESSION["checker1ID"]="empty.PNG";
				}
				else
				{
					$_SESSION["checker1ID"]=($rAName["CheckedBy"]).".png";
				}
			}
...
//echo $checker1;
$que_setting="SELECT Column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = N'LEVEL'";
$r_setting=mysqli_query($db, $que_setting);
if(mysqli_num_rows($r_setting) > 0)
{
	while($rs = mysqli_fetch_array($r_setting))
	{
		$getApprover="SELECT  * FROM `LEVEL` where `".$rs["Column_name"]."` is not null  and `level_type`='OPWP'";
		$rApprovers=mysqli_query($db, $getApprover);
		if(mysqli_num_rows($rApprovers)>0)
		{
			
			while($rs1 = mysqli_fetch_array($rApprovers))
			{
				$getASMID="SELECT distinct CONCAT (`FirstName`,' ',`LastName`) as'Name',C.* FROM `OUSR` A
								left join LEVEL B on  `".$rs["Column_name"]."`= A.`ID`
								left join OPWP C on  B.`level_type`= C.`Document`
								where A.`ID`='".$rs1[$rs["Column_name"]]."' and C.DocId='$key'";
				$rASMID=mysqli_query($db, $getASMID);
				if(mysqli_num_rows($rASMID)>0)
				{
					while($rAID = mysqli_fetch_array($rASMID))
					{
						if($rs["Column_name"]=="checker2")
						{
							if($rAID["CheckedBy2_status"]=="0" || $rAID["CheckedBy2_status"]==0)
							{
								$_SESSION["checker2ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["checker2ID"]=$rs1[$rs["Column_name"]].".png";
							}
							$_SESSION["checker2"]=$rAID["Name"];
							
							
						}
						
						else if($rs["Column_name"]=="checker3")
						{
							$_SESSION["checker3"]=$rAID["Name"];
							
							if($rAID["CheckedBy3_status"]=="0" || $rAID["CheckedBy3_status"]==0)
							{
								$_SESSION["checker3ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["checker3ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="checker4")
						{
							$_SESSION["checker4"]=$rAID["Name"];
							
							
							if($rAID["CheckedBy4_status"]=="0" || $rAID["CheckedBy4_status"]==0)
							{
								$_SESSION["checker4ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["checker4ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="checker5")
						{
							$_SESSION["checker5"]=$rAID["Name"];
							
							if($rAID["CheckedBy5_status"]=="0" || $rAID["CheckedBy5_status"]==0)
							{
								$_SESSION["checker5ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["checker5ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="Approver1")
						{
							$_SESSION["Approver1"]=$rAID["Name"];
							
							if($rAID["ApprovedBy_status"]=="0" || $rAID["ApprovedBy_status"]==0)
							{
								$_SESSION["Approver1ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["Approver1ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="Approver2")
						{
							$_SESSION["Approver2"]=$rAID["Name"];
							
							
							if($rAID["ApprovedBy2_status"]=="0" || $rAID["ApprovedBy2_status"]==0)
							{
								$_SESSION["Approver2ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["Approver2ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="Approver3")
						{
							$_SESSION["Approver3"]=$rAID["Name"];
							
							if($rAID["ApprovedBy3_status"]=="0" || $rAID["ApprovedBy3_status"]==0)
							{
								$_SESSION["Approver3ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["Approver3ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="Approver4")
						{
							$_SESSION["Approver4"]=$rAID["Name"];
							
							if($rAID["ApprovedBy4_status"]=="0" || $rAID["ApprovedBy4_status"]==0)
							{
								$_SESSION["Approver4ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["Approver4ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
						
						else if($rs["Column_name"]=="Approver5")
						{
							$_SESSION["Approver5"]=$rAID["Name"];
							
							if($rAID["ApprovedBy5_status"]=="0" || $rAID["ApprovedBy5_status"]==0)
							{
								$_SESSION["Approver5ID"]="empty.PNG";
							}
							else
							{
								$_SESSION["Approver5ID"]=$rs1[$rs["Column_name"]].".png";
							}
						}
	...
	class PDF extends FPDF{
		...
		function viewTable($db){
			...
			$query_Objectives='';
			$getPWPObjectives="SELECT *
			FROM `OPWP` left join `OBJT` on `OBJT`.`DocId`= `OPWP`.`DocId` and `OBJT`.`Document`= `OPWP`.`Document`
			...
			$query_Mechanics='';
			$getPWPDetails2="SELECT *
			FROM `OPWP` left join `PWP2` on `PWP2`.`DocId`= `OPWP`.`DocId` and `PWP2`.`Document`= `OPWP`.`Document`
			where `OPWP`.`DocId` ='".$_SESSION["key"]."' and `OPWP`.`Document`='".$_SESSION["ObjCode"]."'";
			...
			$getPWPDetails3="SELECT *
			FROM `OPWP` left join `PWP3` on `PWP3`.`DocId`= `OPWP`.`DocId` and `PWP3`.`Document`= `OPWP`.`Document`
			where `OPWP`.`DocId` ='".$_SESSION["key"]."' and `OPWP`.`Document`='".$_SESSION["ObjCode"]."'";
			...
			$getPWPDetails4="SELECT *
			FROM `OPWP` left join `PWP4` on `PWP4`.`DocId`= `OPWP`.`DocId` and `PWP4`.`Document`= `OPWP`.`Document`
			where `OPWP`.`DocId` ='".$_SESSION["key"]."' and `OPWP`.`Document`='".$_SESSION["ObjCode"]."'";
