...
if($_POST['type']==1){
	$empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id WHERE ps.status = 'Recommended'";
...
elseif($_POST['type']==2){
	$empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpsubgroupdetails AS psg ON psg.id = pg.subGroupId WHERE (ps.status IN ('Approved')) AND ps.fnlSts=false";
...
elseif($_POST['type']==3){
  // Change Status
	...
	$qry = "UPDATE `pwpstatus` SET `status`='$sStatus', `signer1`='$iEmpId', `dateSigned1`='$dDateTime', `sg3cmnt`='$sComment' WHERE requestId = '$iReqId'";
...
elseif($_POST['type']==9){
	$empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpsubgroupdetails AS psg ON psg.id = pg.subGroupId WHERE (ps.status IN ('Approved')) AND ps.fnlSts=true";
...
elseif($_POST['type']==10){
  // Change Status
	...
	$qry = "UPDATE `pwpstatus` SET `fnlSts`=true WHERE requestId = '$iReqId'";
	$qryRes = $conn->query($qry);
	if ($qryRes)
	{
		echo $qryRes;
