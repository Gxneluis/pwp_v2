...
function getPWPRej(){
  ...
  $empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpgroupdetails AS pgd ON pg.groupId = pgd.id WHERE ps.status = 'Rejected' AND pgd.approvers = '$empId'";
...
if($_POST['type']==1){
	$empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpgroupdetails AS pgd ON pg.groupId = pgd.id WHERE ps.status = 'Reviewed' AND pgd.approvers = '$empId'";
...
elseif($_POST['type']==2){
	$empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpgroupdetails AS pgd ON pg.groupId = pgd.id WHERE (ps.status IN ('Recommended','Approved','forCEO')) AND pgd.approvers = '$empId'";
	$qryRes_sg = $conn->query($qry_sg);
...
elseif($_POST['type']==3){
  // Change Status
	...
	$qry = "UPDATE `pwpstatus` SET `status`='$sStatus', `approver`='$iEmpId', `dateApproved`='$dDateTime', `sg1cmnt`='$sComment' WHERE requestId = '$iReqId'";
