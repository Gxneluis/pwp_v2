...
if($_POST['type']==1){
	...
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpsubgroupdetails AS psg ON psg.id = pg.subGroupId WHERE ps.status = 'Recommended'";
...
elseif($_POST['type']==2){
	$empId = $_SESSION["empId"];
	$qry_sg = "SELECT ps.*, pr.* FROM pwpstatus AS ps LEFT JOIN pwprequest AS pr ON ps.requestId = pr.id LEFT JOIN pwpgrouping AS pg ON pr.empID = pg.employeeId LEFT JOIN pwpsubgroupdetails AS psg ON psg.id = pg.subGroupId WHERE (ps.status IN ('Approved'))";
