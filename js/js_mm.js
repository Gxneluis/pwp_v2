...
function createTbl(sTblName,aData,sEvent){
  if(sEvent==2){ sAction='<button onclick="setFnlSts('+aData.requestId+')">Confirm</button>'; }
  else{ sAction=""; }
  oTbl = document.getElementById(sTblName);
	...
	oC10.innerHTML = '<button onclick="getPWPData('+aData.requestId+','+sEvent+')">View</button>'+sAction;
}
function setFnlSts(iReqId){
		$.ajax({
			url:"query/pwp_mm.php",
			method:"POST",
			data:{ type:"10",iReqId:iReqId },
			success:function(data) {
				console.log(data);
    		location.reload();
			},
...
// gets PWP For Review
function getForReview(sTblName, iType, sEvent, sPagDiv){
  iRun=0;
  $.ajax({
    url:"query/pwp_mm.php",
    method:"POST",
...
$(document).ready(function() {
	document.getElementById("idBtn_001_mm").addEventListener("click", function () { chngTab(event, "idTab001"); });
	document.getElementById("idBtn_002_mm").addEventListener("click", function () { chngTab(event, "idTab002"); });
  document.getElementById("idBtn_003_mm").addEventListener("click", function () { chngTab(event, "idTab003"); });
  document.getElementById("idBtn_004_mm").addEventListener("click", function () { chngTab(event, "idTab004"); });
  ...
  getForReview("idTbl_mm_001", 1, 0, "idPgng1");
  getForReview("idTbl_mm_002", 8, 1, "idPgng2");
  getForReview("idTbl_mm_003", 2, 2, "idPgng3");
  getForReview("idTbl_mm_004", 9, 1, "idPgng4");
});
