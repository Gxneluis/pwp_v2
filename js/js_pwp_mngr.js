...
function toggleForm(bForm){
  if(bForm == true){ $("#idModal_PWPDetails").css('display','block'); }
  else{ $("#idModal_PWPDetails").css('display','none'); }
}
...
function insertPVA(aData){
	...
	oCell0.innerHTML = '<input type="text" class="cCateBrand"  name="txtCateBrand" value="'+aData.cateBrand+'" >';
	oCell1.innerHTML = '<input type="text" class="cAMSInCase"  name="txtAMSInCase" value="'+aData.amsInCase.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell2.innerHTML = '<input type="text" class="cAMSPesVal"  name="txtAMSPesVal" value="'+aData.amsPeso.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell3.innerHTML = '<input type="text" class="cITSInCase"  name="txtITSInCase" value="'+aData.itsInCase.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell4.innerHTML = '<input type="text" class="cITSPesVal"  name="txtITSPesVal" value="'+aData.itsPeso.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell5.innerHTML = '<input type="text" class="cMAIInCase"  name="txtMAIInCase" value="'+aData.maiInCase.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell6.innerHTML = '<input type="text" class="cMAIPesVal"  name="txtMAIPesVal" value="'+aData.maipvPeso.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell7.innerHTML = '<input type="text" class="cTPDInCase"  name="txtTPDInCase" value="'+aData.tpdInCase.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell8.innerHTML = '<input type="text" class="cTPDPesVal"  name="txtTPDPesVal" value="'+aData.tpdPeso.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" >';
	oCell9.innerHTML = '-';
...
function insertPCE(aData){
	...
	oCell1.innerHTML = '<input type="text" class="cItmDsc"  name="txtItemDesc" value="'+aData.itemDesc+'">';
	oCell2.innerHTML = '<input type="text" class="cItmQty"  name="txtQty" value="'+aData.pcQty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'">';
	oCell3.innerHTML = '<input type="text" class="cItmUnt"  name="txtUnitCost" value="'+aData.pcUnitCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'">';
	oCell4.innerHTML = '<input type="text" class="cItmTtl"  name="txtTotlCost" value="'+aData.pcTotalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'">';
	oCell5.innerHTML = '<input type="text" class="cExpDsc"  name="txtExpeDesc" value="'+aData.expenseDesc+'">';
	oCell6.innerHTML = '<input type="text" class="cExpQty"  name="txtExpeQty" value="'+aData.pceQty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'">';
	oCell7.innerHTML = '<input type="text" class="cExpUnt"  name="txtExpeUnitCost" value="'+aData.pceUnitCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" readOnly >';
	oCell8.innerHTML = '<input type="text" class="cExpTtl"  name="txtExpeTotlCost" value="'+aData.pceTotalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" readOnly >';
	oCell9.innerHTML = '-';
}
// computes Total
...
function fillForm(aData){
	...
	document.getElementById("idChaCla").value = getChaCla(aData.channelClass);
...
function getChaCla(iChaCla){
	sChaCla = "";
	switch(iChaCla){
	  case "1": sChaCla = "CONVENIENCE STORE"; break;
		case "2": sChaCla = "DISTRIBUTOR"; break;
		case "3": sChaCla = "DRUG STORE"; break;
		case "4": sChaCla = "GROCERY"; break;
		case "5": sChaCla = "HRI"; break;
		case "6": sChaCla = "MARKET STALL"; break;
		case "7": sChaCla = "OFFICE SALES"; break;
		case "8": sChaCla = "SARI SARI STORE LARGE"; break;
		case "9": sChaCla = "SARI SARI STORE MEDIUM"; break;
		case "10": sChaCla = "SARI SARI STORE SMALL"; break;
		case "11": sChaCla = "SUPERMARKET A"; break;
		case "12": sChaCla = "SUPERMARKET B"; break;
		case "13": sChaCla = "SUPERMARKET C"; break;
		case "14": sChaCla = "WHOLESALER"; break;
		default: sChaCla = "Code not Found";
	}
	return sChaCla;
}
...
$(document).ready(function() {
	document.getElementById("idBtn_Pending_mngr").addEventListener("click", function () { chngTab(event, "cDiv_pending_mngr"); });
	document.getElementById("idBtn_Previous_mngr").addEventListener("click", function () { chngTab(event, "cDiv_preview_mngr"); });
  document.getElementById("idBtn_Rjct_mngr").addEventListener("click", function () { chngTab(event, "cDiv_Rjct_mngr"); });
	document.getElementById("idBtn_Approved").addEventListener("click", function () { chngStatus("Recommended"); });
	document.getElementById("idBtn_Rejected").addEventListener("click", function () { chngStatus("Rejected"); });

  getForReview("idTbl_Pend_mngr", 1, 0);
	getForReview("idTbl_Prev_mngr", 2, 1);
	getForReview("idTbl_Rjct_mngr", 8, 1);
});
