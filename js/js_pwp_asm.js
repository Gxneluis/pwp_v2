...
function insertPCE(aData){
	...
	oCell2.innerHTML = '<input type="text" class="cItmQty"  name="txtQty" value="'+aData.pcQty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" readOnly >';
	oCell3.innerHTML = '<input type="text" class="cItmUnt"  name="txtUnitCost" value="'+aData.pcUnitCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" readOnly >';
	oCell4.innerHTML = '<input type="text" class="cItmTtl"  name="txtTotlCost" value="'+aData.pcTotalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" readOnly >';
	oCell5.innerHTML = '<input type="text" class="cExpDsc"  name="txtExpeDesc" value="'+aData.expenseDesc+'" readOnly >';
	oCell6.innerHTML = '<input type="text" class="cExpQty"  name="txtExpeQty" value="'+aData.pceQty.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')+'" readOnly >';
...
function fillForm(aData){
	...
	document.getElementById("idPrjTyp").value = getPrjTyp(aData.projType);
...
function getPrjTyp(idPrjTyp){
	sPrjTyp = "";
	switch(idPrjTyp){
		case "1": sPrjTyp = "ADDITIONAL DISCOUNT"; break;
		case "2": sPrjTyp = "DISPLAY ALLOWANCE"; break;
		case "3": sPrjTyp = "ECOM EXPENSE"; break;
		case "4": sPrjTyp = "EVENT SUPPORT"; break;
		case "5": sPrjTyp = "INCENTIVE"; break;
		case "6": sPrjTyp = "IN-STORE PROMOTION"; break;
		case "7": sPrjTyp = "LISTING COST"; break;
		case "8": sPrjTyp = "MANPOWER EXPENSE"; break;
		case "9": sPrjTyp = "MERCHANDISING"; break;
		case "10": sPrjTyp = "PENALTY"; break;
		case "11": sPrjTyp = "VOLUME ENHANCEMENT"; break;
		default: sPrjTyp = "Code not Found";
	}
	return sPrjTyp;
}
...
function chngStatus(sStatus){
	...
	if (sComment!=null){
		$.ajax({
			url:"query/pwp_asm.php",
			method:"POST",
			data:{ type:"3",sStatus:sStatus,sComment:sComment },
			success:function(data) {
				console.log(data);
			},
  		error: function(XMLHttpRequest, textStatus, errorThrown) {
...
$(document).ready(function() {
	document.getElementById("idBtn_Pend_asm").addEventListener("click", function () { chngTab(event, "cDiv_pend_asm"); });
	document.getElementById("idBtn_Prev_asm").addEventListener("click", function () { chngTab(event, "cDiv_prev_asm"); });
	document.getElementById("idBtn_Rjct_asm").addEventListener("click", function () { chngTab(event, "cDiv_Rjct_asm"); });
	document.getElementById("idBtn_Reviewed").addEventListener("click", function () { chngStatus("Reviewed"); });
  ...
  getForReview("idTbl_Pend_asm", 1, 0);
	getForReview("idTbl_Prev_asm", 2, 1);
	getForReview("idTbl_Rjct_asm", 8, 1);
});
